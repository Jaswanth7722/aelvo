...
def html_to_text(html: str) -> str:
    """Extract readable text with selectolax when available, regex fallback otherwise."""
    if HTMLParser:
        ...
        if tree.body:
            ...
            return re.sub(r'\n{3,}', '\n\n', raw_text).strip()
        return "No structural content found."
    stripped = re.sub(r"(?is)<(script|style|noscript|iframe|svg).*?>.*?</\1>", "", html)
    stripped = re.sub(r"(?s)<[^>]+>", "\n", stripped)
    stripped = re.sub(r"\n{3,}", "\n\n", stripped)
    return re.sub(r"[ \t]{2,}", " ", stripped).strip() or "No structural content found."
...
class AelvoSpider(scrapy.Spider if scrapy else object):
    ...
    async def parse(self, response):
        ...
        # --- HTML PARSING (Selectolax Speed Run) ---
            ...
            final_text = html_to_text(response.text)
            MAX_CHARS = 25000
