...
try:
    ...
    class FileLockImpl:  # type: ignore[no-redef]
        _locks: dict = {}
...
class AelvoKernel:
    ...
    def _execute_restore(self, args):
        """Fix 2: Manifest Integrity Validation Before Restore."""
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Online Restore using SQLite backup API (Windows-safe, no connection close needed)
            src_db_path = os.path.join(snap_path, "memory.db")
            with self.db_lock:
                try:
